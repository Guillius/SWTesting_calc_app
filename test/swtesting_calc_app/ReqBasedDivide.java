...
public class ReqBasedDivide {
    ...
    public void testDivByOnePrecision(){
        ...
        assertEquals( 5, divByOne.getAnswer(), 0.000000001 );
    ...
    public void testEquivalencePartitions(){
        ...
        test = new TwoOperandCalculation( "/", -5, 2 );
        assertEquals( 0, test.execute() );
        assertEquals( -2.5, test.getAnswer(), 0.001 );
        
        test = new TwoOperandCalculation( "/", 8, -2 );
        assertEquals( 0, test.execute() );
        assertEquals( -4, test.getAnswer(), 0.001 );
        
        test = new TwoOperandCalculation( "/", 12, 2 );
        assertEquals( 0, test.execute() );
    ...
    @Test 
    public void testDivisionSpecialCases(){
        ...
        test = new TwoOperandCalculation( "/", NEGATIVE_INFINITY, POSITIVE_INFINITY );
        assertEquals( 0, test.execute() );
        assertEquals( NaN, test.getAnswer(), 0.001 );
